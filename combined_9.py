...
FEEDER_SERVO  = 0
SORT_SERVO_8  = 8    # Stage 1 — splits Left (Purple/Blue) vs Right (Yellow/Red)
SORT_SERVO_4  = 4    # Stage 2 Left — splits Purple vs Blue
SORT_SERVO_12 = 12   # Stage 2 Right — splits Yellow vs Red

FEED_PUSH = 180
FEED_REST = 0
kit.servo[FEEDER_SERVO].actuation_range = 180

# ─── Depot order (left to right): Purple | Blue | Yellow | Red ───────────────
#
#  Servo 8:  LEFT  (45°)  → Purple / Blue
#            RIGHT (135°) → Yellow / Red
#
#  Servo 4:  LEFT  (135°) → Purple
#            RIGHT (30°)  → Blue
#
#  Servo 12: LEFT  (30°)  → Yellow
#            RIGHT (135°) → Red
#
# Adjust these angles to match your physical build.
...
# ─── Timing ──────────────────────────────────────────────────────────────────
COOLDOWN   = 3.5   # Seconds between sort cycles (lets ball clear the system)
HOLD_TIME  = 0.5   # Seconds a colour must be seen continuously before acting
...
def reset_gates():
    """Return all gates to neutral (90 degrees)."""
...
try:
    while True:
        ...
        # HSV masks — dual-range red wraps the 0/180 hue boundary
        masks = {
            "Red": cv2.bitwise_or(
